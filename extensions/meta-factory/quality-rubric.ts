...
export const WEIGHTS = {
  ...
  coverage: 5,
  /** Definition configs were generated by swamp creation commands, not copied. */
  creation: 5,
...
export interface ScoreResult {
  ...
  nextActions: string[];
  /** Definition-config issues (creation-command rule), for the report. */
  definitionIssues: DefinitionIssueSummary[];
}
...
/**
 * Definition-config structure issues contributed by `definitions-lint.ts`.
 *
 * Kept structural (not the linter's own type) so the rubric stays free of a
 * filesystem dependency and callers can pass plain JSON.
 */
export interface DefinitionIssueSummary {
  /** `error` breaks the creation-command rule; `warning` is advisory. */
  severity: "error" | "warning";
  /** Stable rule id (`id-missing`, `id-duplicate`, …). */
  rule: string;
  /** Human-readable description of the problem. */
  message: string;
  /** Repo-relative path of the offending definition. */
  path?: string;
}

/**
 * Score whether the extension's definition configs were generated by the
 * matching swamp creation command.
 *
 * Only **structural** problems fail this check, because they are provable from
 * the file alone: a missing `id:` (hand-written), a non-UUID `id:`
 * (fabricated), or a reused `id:` (copied). Advisory warnings — the best-effort
 * `create-unconfirmed` audit signal, or a missing optional `name`/`typeVersion`
 * — are reported for the operator but cost no points, so a legitimate
 * definition is never penalised by an absent or incomplete audit timeline. Any
 * error costs the whole check.
 */
export function checkDefinitions(
  issues: DefinitionIssueSummary[],
): CheckResult {
  const max = WEIGHTS.creation;
  ...
  if (errors.length === 0) {
    return {
      id: "creation",
      label: "Definitions created by swamp creation commands",
      earned: max,
      max,
      status: "pass",
      note: warnings.length === 0
        ? "every definition carries a generated, unique id"
        : `${warnings.length} advisory warning(s), no structural problems: ${
          warnings
            .slice(0, 3)
            .map((i) => `${i.path ? `${i.path}: ` : ""}${i.rule}`)
            .join("; ")
        }`,
    };
  }
  ...
  return {
    ...
    max,
    status: "fail",
    note: `${errors.length} error(s): ${detail} — ` +
      `regenerate hand-written or copied definitions with the matching ` +
      `\`swamp model create\` / \`swamp workflow create\` / \`swamp vault create\``,
  };
}
...
export interface ScoreInput {
  ...
  lintStdout?: string;
  /** Definition-config issues from `definitions-lint.ts`. */
  definitionIssues?: DefinitionIssueSummary[];
...
export function scoreExtension(input: ScoreInput): ScoreResult {
  ...
  const checks: CheckResult[] = [
    ...
    coverage.check,
    checkDefinitions(input.definitionIssues ?? []),
  ...
  return {
    ...
    missing,
    nextActions,
    definitionIssues: input.definitionIssues ?? [],
  };
}
